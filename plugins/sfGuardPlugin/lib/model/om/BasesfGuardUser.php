...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
  ...
  const PEER = 'sfGuardUserPeer';
	...
	/**
	 * @var        array BlogArticle[] Collection to store aggregation of BlogArticle objects.
	 */
	protected $collBlogArticles;

	/**
	 * @var        Criteria The criteria used to select the current contents of collBlogArticles.
	 */
	private $lastBlogArticleCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collBlogArticles = null;
			$this->lastBlogArticleCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collBlogArticles !== null) {
				foreach ($this->collBlogArticles as $referrerFK) {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getBlogArticles() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addBlogArticle($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collBlogArticles collection (array).
	 *
	 ...
	 * @see        addBlogArticles()
	 */
	public function clearBlogArticles()
	{
		$this->collBlogArticles = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collBlogArticles collection (array).
	 *
	 * By default this just sets the collBlogArticles collection to an empty array (like clearcollBlogArticles());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initBlogArticles()
	{
		$this->collBlogArticles = array();
	}

	/**
	 * Gets an array of BlogArticle objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related BlogArticles from storage. If this sfGuardUser is new, it will return
	 ...
	 * @throws     PropelException
	 */
	public function getBlogArticles($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collBlogArticles === null) {
			if ($this->isNew()) {
			   $this->collBlogArticles = array();
				...
				$criteria->add(BlogArticlePeer::USER_ID, $this->id);

				BlogArticlePeer::addSelectColumns($criteria);
				$this->collBlogArticles = BlogArticlePeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				BlogArticlePeer::addSelectColumns($criteria);
				if (!isset($this->lastBlogArticleCriteria) || !$this->lastBlogArticleCriteria->equals($criteria)) {
					$this->collBlogArticles = BlogArticlePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastBlogArticleCriteria = $criteria;
		return $this->collBlogArticles;
	}

	/**
	 * Returns the number of related BlogArticle objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related BlogArticle objects.
	 * @throws     PropelException
	 */
	public function countBlogArticles(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collBlogArticles === null) {
			if ($this->isNew()) {
				...
				$count = BlogArticlePeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastBlogArticleCriteria) || !$this->lastBlogArticleCriteria->equals($criteria)) {
					...
					$count = count($this->collBlogArticles);
				}
			} else {
				$count = count($this->collBlogArticles);
			}
	...
	/**
	 * Method called to associate a BlogArticle object to this object
	 * through the BlogArticle foreign key attribute.
	 *
	 * @param      BlogArticle $l BlogArticle
	 * @return     void
	 * @throws     PropelException
	 */
	public function addBlogArticle(BlogArticle $l)
	{
		if ($this->collBlogArticles === null) {
			$this->initBlogArticles();
		}
		if (!in_array($l, $this->collBlogArticles, true)) { // only add it if the **same** object is not already associated
			array_push($this->collBlogArticles, $l);
	...
	{
		if ($deep) {
			if ($this->collBlogArticles) {
				foreach ((array) $this->collBlogArticles as $o) {
